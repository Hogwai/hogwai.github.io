...
- Memory: A compiled Pattern is one of the most memory-intensive Java objects<sup><a href="#ref-demystifying-java-object-sizes-compact-h">[1]</a></sup>.
...
As stated in the javadoc<sup><a href="#ref-stringmatchesstring-regex">[2]</a></sup>:
...
- `RegExUtils.java`<sup><a href="#ref-regexutilsjava">[3]</a></sup>: Utility class providing methods like `replaceFirst` or `replaceAll`
...
- Force backtracking: Java's regex engine is NFA-based and normally includes memoization optimizations (JDK-6328855)<sup><a href="#ref-jdk-6328855-patternmatches-performance-i">[4]</a></sup> to mitigate exponential runtime. These optimizations are explicitly disabled when backreferences are present, because backreferences cannot be modeled in a DFA at all.
...
Modern JDK includes memoization optimizations (JDK-6328855)<sup><a href="#ref-jdk-6328855-patternmatches-performance-i">[4]</a></sup> that mitigate simple cases, but the gap grows exponentially with input length. At 30+ characters, the difference becomes astronomical. And when backreferences are present, the mitigations are disabled entirely.
...
When you need to test many strings against the same pattern, these methods work with the collections/streams API without wrapping in a lambda<sup><a href="#ref-patternasmatchpredicate-java-11-api-docs">[6]</a></sup>:
...
Java 21 introduced `Pattern.splitWithDelimiters()`<sup><a href="#ref-jdk-8305486-add-splitwithdelimiters-meth">[7]</a></sup> and its `String` counterpart. Unlike `split()`, which discards the delimiters, this method returns both the substrings and the delimiters interleaved:
...
- <a id="ref-jdk-6328855-patternmatches-performance-i"></a>[JDK-6328855: Pattern.matches() performance issues with exponential runtime](https://bugs.openjdk.org/browse/JDK-6328855)
- <a id="ref-patternjava-openjdk-patternquote-impleme"></a>[Pattern.java: OpenJDK Pattern.quote() implementation](https://github.com/openjdk/jdk/blob/master/src/java.base/share/classes/java/util/regex/Pattern.java#L1500)
- <a id="ref-patternasmatchpredicate-java-11-api-docs"></a>[Pattern.asMatchPredicate(): Java 11+ API docs](<https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/regex/Pattern.html#asMatchPredicate()>)
- <a id="ref-jdk-8305486-add-splitwithdelimiters-meth"></a>[JDK-8305486: Add splitWithDelimiters methods to Pattern and String](https://bugs.openjdk.org/browse/JDK-8305486)
